...
<script>
    ...
    import { settlementList, userSettlementList } from 'api/financial/pay_settlement'
...
//            this.getAppList();
        ...
        methods: {
            getList() {
                ...
                userSettlementList(search, page, size, sort).then(response => {
            ...
            handleDownload() {
                require.ensure([], () => {
                    ...
                    settlementList({}, 1, 1000, null).then(response => {
                        ...
                        const tHeader = ['序号', '商户名称', '订单总金额', '应结总金额', '订单时间','创建时间'];
                        const filterVal = [
                            { name: 'id' },
                            { name: 'userName' },
                            { name: 'totalMoneyYuan' },
